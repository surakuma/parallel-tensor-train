...
%%Tables~\ref{tab:12-dim-10-3} and ~\ref{tab:12-dim-10-6} assess efficacy of all the proposed heuristics  for this setting.
...
\begin{figure}[!htb]
	\begin{center}
		\includegraphics[scale=0.35]{./ratio_to_minimal-4-12.pdf}
	\end{center}
\caption{Comparison of ratio to minimal for $12$-dimensional tensors.\label{fig:rtm-12}}
\end{figure}

\noindent Figure~\ref{fig:rtm-12} depicts ratio to minimal values of all the considered heuristics for 12-dimensional tensors.
...
\end{figure}
Ratio to minimal values of the heuristics are for $6$- dimensional tensors are shown in Figure~\ref{fig:rtm-6}.
\todo[inline]{SK: explain technical analysis of Figures~\ref{fig:rtm-12} and ~\ref{fig:rtm-6}.}  

\subsection{Evaluation for Molecular Chemsitry Data}


As we observe in previous tables, $H3$ is the best among all our proposed heuristics. Hence here we only perform comparison of $OTTA$ and $H3$ for tensors arising in molecular chemistry simulations. These are $4$-dimensional tensors of different sizes and are obtained by choosing basis sets of $H_2O$, $H_2$ and $CH_4$ molecules. Table~\ref{tab:dim-4-mol-chem} shows values of different performance metrics for these tensors. Exp1, Exp2 and Exp3 correspond to simulations with $H_2O$ molecule, while Exp4 and Exp5 correspond to simulations with $H_2$ and $CH_4$ respectively.  
...
\todo[inline]{SK: present details of Table~\ref{tab:dim-4-mol-chem}.}
