...
%%%%%
\institute{Alpines, Inria Paris, France\\
...
Let $||\tensor{A}||_F$ denote the frobenius norm of a $d$-dimensional tensor \tensor{A} and it is defined as, $||\tensor{A}||_F=$ $\sqrt{\sum_{i_1, i_2, \cdots, i_d}A(i_1,i_2,\cdots, i_d)^2 }$. Similarly, the frobenius norm of a matrix $A$ is defined as, $||A||_F = \sqrt{\sum_{i,j} A(i;j)^2}$. Let $tr(A)$ denote the trace of a square matrix $A$ and it is computed as, $tr(A) = \sum_i A(i;i)$.
...
\begin{enumerate}
	...
	&\qquad\le \frac{\epsilon^2}{d-1} + ||E_C||_F^2 + ||E_B||_F^2 ||\Sigma_\alpha||_F^2 \\
	&\qquad= \frac{\epsilon^2}{d-1} + \delta^2 (d_2-1)+ \delta^2 (d_1-1) tr(\Sigma_\alpha^2)\le \epsilon^2.
	\end{align*}
	\noindent After simplifying this expression we obtain, $\delta \le \epsilon \sqrt{\frac{d-2}{(d-1) (d_2 -1 + (d_1-1) tr(\Sigma_\alpha^2))}}$.
...
Figure~\ref{fig:rtm-12} depicts $r$ values of all the considered approaches for 12-dimensional tensors. All the approaches achieve the same compression ratio for $Sin$ function, hence their $r$ values are $1$. $r_ {\hthird}$ and $r_ {\otta}$ values are almost same and are equal to $1$. For the prescribed accuracy of $10^{-3}$, $r_ {\hfirst}$ values are between $2.5$ to $3.6$ for the other functions. \hsecond compression ratios are slightly better than \hfirst, hence $r_ {\hsecond}$ values are smaller than $r_ {\hfirst}$, but still in the range of $2.25$ to $3.25$. For the prescribed accuracy of $10^{-6}$, $r_ {\hfirst}$ value for $Log$ function is more than $25$. This indicates that \hfirst stores $25\times$ more entries than the best approach. $r_ {\hfirst}$ and $r_ {\hsecond}$ are similar for $ISR$, $ICR$ and $IPR$ functions and are close to $5$. 
...
Figure~\ref{fig:rtm-6} shows $r$ values of all the approaches for $6$-dimensional tensors. Similar to the previous plot, $r$ values of all the approaches for $Sin$ function are $1$. $ {\hthird}$ and $ {\otta}$ achieve similar compression, hence their values are equal and close to $1$. For the prescribed accuracy of $10^{-3}$, $r_ {\hfirst}$ value of $Log$ function is more than $3.5$. Therefore \hfirst requires $3\times$ more storage than the best approach for this function. For the other functions, its values are between $1.75$ to $2.25$. $r_ {\hsecond}$ values are slightly better than $r_ {\hfirst}$, and are in the range of $1.75$-$2.25$. For the prescribed accuracy of $10^{-6}$, \hfirst and \hsecond require $10\times$ and $6\times$ more storage than the best approach for $Log$ function. \hfirst and \hsecond achieve close compression ratios for $ISR$, $ICR$ and $IPR$ functions. Hence their $r$ values are also close.
